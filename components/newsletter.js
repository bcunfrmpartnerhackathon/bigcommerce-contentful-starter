...
const Newsletter = ({ component, ...otherProps }) => {
  // Extract our module data
  const { title, id, terms, submit, successMsg, errorMsg } = otherProps?.entry || {};
    ...
    <>
      ...
      <form className="form" onSubmit={handleSubmit(onSubmit)}>
        <AnimatePresence exitBeforeEnter>
          {!error && !success && (
            <m.div initial="hide" animate="show" exit="hide" variants={fadeAnim} className="form--fields">
              <input
                type="text"
                name="fullname"
                autoComplete="off"
              ...
              {terms && (
                <div className="control">
                  ...
                  />
                  <label htmlFor={`acceptTerms-${id}`} className="control--label for-checkbox">
                    <Icon name="Checkmark" />
          ...
          {success && (
            ...
            >
              <div className="form--success-content">
                {successMsg ? <BlockContent blocks={successMsg} /> : <h2>Success!</h2>}
              </div>
            </m.div>
          )}
